 ...
 */
 module.exports = (function Feliz(conf={}){
    ...
    // if not instantiated, return the pseudo-instance after the first call.
    ...
    /**
     * The latest core module loaded.
     * @name module:feliz#lastCore
     * @private
     * @type string
     */
    this.set('latestCore', null, {writable:true, enumerable:false});
    ...
    const module$ = Observable
        ...
        .concatMap(path => {
            ...
            const name = PATH
                ...
                .replace(/^\d+\-/, '')
            feliz.latestCore = name;
            // When the events module is ready, start auto triggering.
            if (feliz.events && feliz.events.constructor.name == 'Events')
                feliz.events.emit(`core:${name}~before`);
            return func.call(feliz, {name, path, conf})
        })
        .do(instance => {
            feliz = instance;
            if (feliz.events && feliz.events.constructor.name == 'Events')
                feliz.events.emit(`core:${feliz.latestCore}`);
        })
