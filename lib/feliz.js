 ...
 */
 module.exports = (function feliz(conf={}){
    ...
    if (this.set !== undefined) return Observable.of(this);
     ...
     */
    this.set('events', new (class Events extends EVENTS {}), {enumerable:false})
    ...
    const feliz$ = Observable
        .of(PATH.join(__dirname, 'core'))
        .mergeMap(path => this.util.rx.path(path).readdir())
        .toArray()
        .mergeMap(paths => paths.sort())
        .mergeMap(path => {
            const mod = require(path);
            if (!this.util.is(mod).function()) throw this.error.type({
                name: 'feliz.core',
                type: 'Function',
                data: mod
            });
