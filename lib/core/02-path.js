...
module.exports = function path(info){
    ...
    this.events.on('core:conf.setter', function(conf){
        if (!conf.path) return;
        this.path = conf.path;
    });
...
function converter(info, target){ return Object
    .keys(target)
    ...
    .reduce((obj, cur)=> {
        ...
        const args = cur.args.map(arg => this.util
            .string(arg)
            .template(contextualizer.call(this, info))
        );
...
function contextualizer(info){
    const value = this.util
        ...
        .merge(this.path);
    this.debug(`${info.name}.context`, value);
    return value;
}
...
function setter(info, target){
    let value = this.util.object(this.conf.path).merge(target)
    this.debug(`${info.name}.set:init`, value);
    validator.call(this, info, value);
    value = converter.call(this, info, value);
    this.debug(`${info.name}.set:done`, value);
