...
function Debugger(type, ...params){
    ...
    if (type === null) type = '';
    if (!this.util.is(type).string())
        throw this.error.type({ name:'debug:type', type:'String', data:type });
...
function Handler(type, ...params){
    ...
    params = params.reduce((result, param, i) => {
        ...
        if (!is.objectish() && !is.arrayish()) result.push(param);
    ...
    // add a trailing new line when not tracing
    if (!process.env.TRACER) params.push('\n');
